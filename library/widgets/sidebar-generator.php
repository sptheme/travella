...
if( ! class_exists( 'avia_sidebar' ) )
{
	class avia_sidebar
	{
		...
		{
		    $this->paths['css']   = SP_BASE_URL . 'library/widgets/css/';
		    $this->paths['js']    = SP_BASE_URL . 'library/widgets/js/';
		    $this->stored         = 'avia_sidebars';
		    $this->title          = __('Add new sidebar','sptheme_admin');
		...
		function template_add_widget_field()
		{
            ...
            echo "\n  <h3>". $this->title ."</h3>";
            echo "\n    <span class='avia_style_wrap'><input type='text' value='' placeholder = '".__('Enter Name of the new Sidebar here')."' name='avia-add-widget' /></span>";
            echo "\n    <input class='button button-primary button-large' type='submit' value='".__('Add Sidebar')."' />";
            echo "\n    ".$nonce;
            echo "\n  </form>";
            echo "\n</script>\n";
		}
